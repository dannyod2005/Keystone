...
:root{
  ...
  --gold:#C4922F; --gold-dark:#96701F; --gold-tint:#F3E7CE;
  --slate:#5B6472; --slate-light:#8B93A0; --line:#E4E0D6;
  --success:#3F7D58; --success-tint:#E4EFE8; --coral:#C1573D; --coral-tint:#F6E5DF;
  --font-display:'Fraunces', serif; --font-body:'Inter', sans-serif; --font-mono:'IBM Plex Mono', monospace;
}
.ks-root{ font-family:var(--font-body); color:var(--ink); background:var(--paper); min-height:100%; }
.ks-root *{ box-sizing:border-box; }
.ks-btn{ font-family:var(--font-body); font-weight:600; font-size:14px; border-radius:8px; padding:10px 18px; cursor:pointer; border:none; display:inline-flex; align-items:center; gap:6px; transition:opacity .15s ease, transform .1s ease; }
.ks-btn:active{ transform:scale(0.98); }
.ks-btn:hover{ opacity:0.88; }
.ks-btn-primary{ background:var(--ink); color:var(--paper); }
.ks-btn-gold{ background:var(--gold); color:#2B1E06; }
.ks-btn-ghost{ background:transparent; color:var(--ink); border:1px solid var(--line); }
.ks-btn-ghost:hover{ background:var(--paper-2); opacity:1; border-color:var(--slate-light); }
.ks-card{ background:var(--paper-2); border:1px solid var(--line); border-radius:14px; }
.ks-badge{ font-size:11px; font-weight:600; letter-spacing:0.02em; padding:3px 9px; border-radius:100px; text-transform:uppercase; }
.ks-hairline{ border:none; border-top:1px solid var(--line); }
.ks-scroll::-webkit-scrollbar{ width:6px; height:6px; }
.ks-scroll::-webkit-scrollbar-thumb{ background:var(--line); border-radius:10px; }
.ks-nav-item{ display:flex; align-items:center; gap:10px; padding:10px 14px; border-radius:8px; font-size:14px; font-weight:500; color:var(--slate); cursor:pointer; transition:background .15s; }
.ks-nav-item:hover{ background:#FFFFFF10; }
.ks-nav-item.active{ background:var(--paper-2); color:var(--ink); }
.ks-input{ font-family:var(--font-body); border:1px solid var(--line); border-radius:9px; padding:10px 14px 10px 40px; font-size:14px; width:100%; background:var(--paper-2); outline:none; }
.ks-input:focus{ border-color:var(--gold); }
.ks-tab{ padding:10px 4px; font-size:14px; font-weight:600; color:var(--slate-light); cursor:pointer; border-bottom:2px solid transparent; }
.ks-tab.active{ color:var(--ink); border-bottom-color:var(--gold); }
`;
...
const LEARNER = { name: "Alex Chen", goal: "AI Product Leadership", streak: 4, minutesThisWeek: 186, dailyGoalMin: 30, goalHitDays: 4 };
...
function KeystoneArch({ progress = 0, size = 44 }) {
...
/* ---------- Marketing top bar (logged-out) ---------- */

function MarketingHeader({ onGo, onAuth }) {
  return (

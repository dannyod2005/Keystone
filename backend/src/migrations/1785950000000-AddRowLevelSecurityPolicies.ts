...
export class AddRowLevelSecurityPolicies1785950000000 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP POLICY IF EXISTS "module_notes_update_own" ON "module_notes"`,
    );
    await queryRunner.query(
      `DROP POLICY IF EXISTS "module_notes_insert_own" ON "module_notes"`,
    );
    await queryRunner.query(
      `DROP POLICY IF EXISTS "module_notes_select_own" ON "module_notes"`,
    );
    await queryRunner.query(
      `ALTER TABLE "module_notes" DISABLE ROW LEVEL SECURITY`,
    );
